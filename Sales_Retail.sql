...
(    
        SELECT EXTRACT(YEAR FROM sale_date) as year,
               ...
               RANK() OVER(PARTITION BY EXTRACT(YEAR FROM sale_date) ORDER BY AVG(total_sale) DESC) as rankFROM retail_sales
GROUP BY 1, 2
) as t1
WHERE rank = 1

--Q.8 Write a SQL query to find the maximum price per unit sold for each category--
SELECT 
    category,
...
--Q.12 Write a SQL query to find the customer with the most orders--
...
--Q.13 Write a SQL query to retrieve all transactions made on a specific date and their corresponding total sales--
SELECT 
    sale_date,
    SUM(total_sale) AS total_sales
FROM retail_sales
